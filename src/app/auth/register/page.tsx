...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      ...
      dateOfBirth: formData.get('dateOfBirth'),      address: formData.get('address'),
    };
    ...
    try {
      ...
      if (isDoctor) {
        router.push('/auth/signin'); // Doctors go to sign in
      } else {
        router.push('/appointments/book'); // Patients go to booking
      }
    } catch (err) {
            ...
            <>              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 px-3 py-2 text-gray-900 dark:text-white focus:border-blue-500 focus:ring-blue-500"
                  placeholder="You can add your address later in your profile"
                />
              </div>
            </>
          )}
          ...
          <div>
            <button
              ...
              className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${
                isDoctor ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'
              } focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                isDoctor ? 'focus:ring-green-500' : 'focus:ring-blue-500'
            ...
            >
              {isSubmitting ? (
                ...
                isDoctor ? 'Register as Doctor' : 'Register & Book Appointment'
              )}
            </button>
          </div>
        </form>
